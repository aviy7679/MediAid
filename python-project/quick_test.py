...
# נניח שיצרת כבר את TextAnalyzer
analyzer = TextAnalyzer(auto_load=True)

# טקסט לבדיקה
text = "The patient suffers from chest pain, fatigue and high fever."

# נביא את כל היישויות שזוהו
entities = analyzer.get_all_entities(text)

# ניגש לבסיס הידע של המודל (CDB)
cdb = analyzer.cat.cdb

print("📋 ישויות שזוהו בטקסט:\n")

for ent_id, data in entities.get("entities", {}).items():
    cui = data.get("cui")
    detected = data.get("detected_name")
    pretty = data.get("pretty_name")
    types = data.get("type_ids", [])
    acc = data.get("acc")

    print(f"🔹 {detected}  →  {pretty}")
    print(f"    - CUI: {cui}")
    print(f"    - סוגים סמנטיים (type_ids): {types}")
    print(f"    - רמת ביטחון: {acc:.2f}")

    # הדפסת מילים נרדפות מתוך בסיס הידע
    if cui in cdb.cui2names:
        synonyms = cdb.cui2names[cui]
        print(f"    - מילים נרדפות: {', '.join(synonyms)}")
    else:
        print(f"    - אין מילים נרדפות מוכרות במודל.")
